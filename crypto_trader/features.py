...
class FeatureEngineer:
    ...
    def add_technical_indicators(self, df: pd.DataFrame, symbol: str | None = None) -> pd.DataFrame:
        """
        ...
        gain = (delta.where(delta > 0, 0)).rolling(window=cfg.rsi_window).mean()
        loss = (-delta.where(delta < 0, 0)).rolling(window=cfg.rsi_window).mean()
        rs = gain / loss
        df['RSI'] = 100 - (100 / (1 + rs))

        # 2. MACD - 使用昨日数据
        exp12 = close_prev.ewm(span=cfg.macd_fast, adjust=False).mean()
        exp26 = close_prev.ewm(span=cfg.macd_slow, adjust=False).mean()
        macd = exp12 - exp26
        df['MACD'] = macd
        df['MACD_Signal'] = macd.ewm(span=cfg.macd_signal, adjust=False).mean()
        df['MACD_Pct'] = macd / close_prev.replace(0, np.nan)

        # 3. Bollinger Bands - 使用昨日数据
        sma20 = close_prev.rolling(window=cfg.bb_window).mean()
        std20 = close_prev.rolling(window=cfg.bb_window).std()
        df['BB_Upper'] = sma20 + (std20 * cfg.bb_std)
        df['BB_Lower'] = sma20 - (std20 * cfg.bb_std)
        df['BB_Width'] = df['BB_Upper'] - df['BB_Lower']
        df['BB_Width_Pct'] = df['BB_Width'] / close_prev.replace(0, np.nan)
        ...
        df['ATR'] = true_range.rolling(window=cfg.atr_window).mean()

        # 6. SMA (50, 200) - 使用昨日数据
        df['SMA_50'] = close_prev.rolling(window=cfg.sma_fast).mean()
        df['SMA_200'] = close_prev.rolling(window=cfg.sma_slow).mean()
        ...
        vol_sma = vol_prev.rolling(window=cfg.volume_window).mean()
        df['Vol_Ratio'] = vol_prev / vol_sma

        # 8. Rolling Volatility - 使用昨日对数收益率
        log_ret_prev = df['Log_Returns'].shift(1)
        df['Rolling_Vol'] = log_ret_prev.rolling(window=cfg.rolling_vol_window).std()
        ...
        df['Range_Pct'] = (high_prev - low_prev) / close_prev.replace(0, np.nan)
        df['Volatility_Regime'] = (
            df['Rolling_Vol'] / df['Rolling_Vol'].rolling(window=cfg.rolling_vol_window).median().replace(0, np.nan)
        )

#!/bin/bash
# Candidate pipeline: include all newly trained experts together, retrain MoE gate, then backtest.
...
GATE_TIMESTEPS="${GATE_TIMESTEPS:-120000}"
...
# All newly trained experts (as requested, including E3 even though not passed).
...
E3_SRC="${E3_SRC:-${PROJECT_ROOT}/checkpoints/moe/candidate_fix_e3e5e2/20260302_200338/E3_PPO_range_calmar/seed_2202/E3_PPO_range_calmar}"
...
CAND_ROOT="${PROJECT_ROOT}/checkpoints/moe/candidate_all_new"
CAND_EXPERTS_ROOT="${CAND_ROOT}/experts_${RUN_ID}"
CAND_GATE_ROOT="${CAND_ROOT}/gate_${RUN_ID}"
ANALYSIS_DIR="${PROJECT_ROOT}/runs/analysis/candidate_all_new/${RUN_ID}"
mkdir -p "${CAND_ROOT}" "${CAND_EXPERTS_ROOT}" "${ANALYSIS_DIR}"

echo "=========================================="
echo " Candidate 8-Expert Test (All New Upgrades)"
echo "=========================================="
echo "RUN_ID: ${RUN_ID}"
...
for p in \
  "${MANIFEST}" "${TRAIN_DATA_PATH}" "${BACKTEST_DATA_PATH}" \
  "${E2_SRC}/model.zip" "${E3_SRC}/model.zip" "${E4_SRC}/model.zip" "${E5_SRC}/model.zip" "${E7_SRC}/model.zip"; do
  if [ ! -f "${p}" ]; then
...
# Build mixed stage1 root with symlinks:
# stable for all, replace E2/E3/E4/E5/E7 with new artifacts.
...
rm "${CAND_EXPERTS_ROOT}/E3_PPO_range_calmar"
...
ln -s "${E3_SRC}" "${CAND_EXPERTS_ROOT}/E3_PPO_range_calmar"
...
echo "[1/4] Retrain MoE gate with all new experts -> ${CAND_GATE_ROOT}"
...
CANDIDATE_JSON="${ANALYSIS_DIR}/candidate_all_new.json"
SUMMARY_JSON="${ANALYSIS_DIR}/summary.json"
BASELINE_PLOT="${ANALYSIS_DIR}/baseline_8exp_curve.png"
CANDIDATE_PLOT="${ANALYSIS_DIR}/candidate_all_new_curve.png"
...
echo "[3/4] Backtest candidate (all new experts + retrained gate)"
M_PATH="${MANIFEST}" \
...
echo ""
echo "✅ Candidate all-new test completed."
